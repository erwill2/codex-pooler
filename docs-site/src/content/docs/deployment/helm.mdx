...
replicas. It uses public placeholders only. It assumes:
...
`image.pullPolicy`, `secrets.create`, `secrets.existingSecret`, app probes,
rollout strategy, termination grace, worker or scheduler replica counts, or
explicit websocket owner-forwarding values.
