...
Kilo uses OpenCode-style `limit.{context,input,output}` fields but includes reasoning tokens in overflow accounting and supports `compaction.threshold_percent` for preflight compaction. `limit.input: 328000` leaves 308k usable input tokens after the default 20k reserve; the optional 75% threshold asks Kilo to compact even earlier.
