...
description: Configure Codex CLI and Codex Desktop with Codex Pooler backend compatibility, shared ~/.codex/config.toml provider settings, HTTP/SSE fallback, optional operator MCP access, and local session migration.
head:
  - tag: script
    attrs:
      type: application/ld+json
    content: |
      {"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Does Codex Pooler work with Codex CLI and Codex Desktop?","acceptedAnswer":{"@type":"Answer","text":"Yes. Codex CLI and Codex Desktop share Codex configuration layers. Put the Codex Pooler provider in ~/.codex/config.toml, set model_provider to codex-pooler-ws, and point base_url at https://codex-pooler.example.com/backend-api/codex."}},{"@type":"Question","name":"Should Codex CLI or Codex Desktop use /v1 with Codex Pooler?","acceptedAnswer":{"@type":"Answer","text":"No. Codex backend clients should use /backend-api/codex. The /v1 surface is for selected OpenAI SDK-compatible clients, not Codex CLI or Codex Desktop backend traffic."}},{"@type":"Question","name":"Where should Codex Pooler provider settings live for Codex?","acceptedAnswer":{"@type":"Answer","text":"Use the user-level ~/.codex/config.toml file for provider and auth settings. Project-local .codex/config.toml files are trust-gated and cannot override machine-local provider keys such as model_provider and model_providers."}}]}
---

![Codex Pooler integration for Codex CLI and Codex Desktop](/codex-pooler-codex.png)

Codex Pooler supports both Codex CLI and Codex Desktop through the Codex backend compatibility route. Both Codex surfaces share the same configuration layers and user-level `~/.codex/config.toml`, so one provider block can serve the terminal and desktop/IDE experience.
...
In Codex Desktop or the IDE extension, open the same file from the settings gear with **Codex Settings > Open config.toml**. CLI users can edit `~/.codex/config.toml` directly.
...
Use the websocket-capable provider for normal Codex backend behavior in Codex CLI and Codex Desktop. Keep `name = "OpenAI"` because Codex uses that provider family for its backend Responses protocol.
