...
When Codex Pooler serves current model metadata, Codex does not need explicit client-side context overrides. If you must pin `gpt-5.6-terra` for an older Codex catalog, use Codex's own fields: `model_context_window = 372000` and `model_auto_compact_token_limit = 334800`. Codex computes an effective 95% window for turn budgeting and does not send an OpenAI SDK-style output cap on normal `/responses` turns.
