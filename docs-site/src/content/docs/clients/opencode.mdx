...
OpenCode subtracts its compaction reserve from `limit.input` before declaring the session full. `228000` leaves 208k usable input tokens after the default 20k reserve, so 208k input plus a 64k output cap stays inside Codex Pooler's 272k `gpt-5.5` window. OpenCode caps request output at 32k by default; set `OPENCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX=64000` only when you want OpenCode to request the full 64k cap.
