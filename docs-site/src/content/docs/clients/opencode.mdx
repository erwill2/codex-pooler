...
OpenCode subtracts its compaction reserve from `limit.input` before declaring the session full. `328000` leaves 308k usable input tokens after the default 20k reserve, so 308k input plus a 64k output cap stays inside Codex Pooler's 372k GPT-5.6 tier windows. OpenCode caps request output at 32k by default; set `OPENCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX=64000` only when you want OpenCode to request the full 64k cap.
