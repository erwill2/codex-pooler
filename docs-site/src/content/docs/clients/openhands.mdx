...
The audited OpenHands app source delegates authoritative LLM limit parsing to `openhands-sdk`; the local app checkout does not expose source-verifiable `gpt-5.6-terra` context or output limit fields. Keep this example to model/base URL environment overrides unless the SDK source is audited too.
