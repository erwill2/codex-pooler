---
title: OpenHands
description: Configure OpenHands CLI with Codex Pooler's narrow OpenAI-compatible /v1 surface, a Pool API key, and env override mode.
---

OpenHands CLI can use Codex Pooler through the narrow OpenAI-compatible `/v1` surface. Keep the Pool API key in the environment, point OpenHands at the `/v1` base URL, and pass `--override-with-envs` so the environment values win for the run.

## Install and run locally

Use the published OpenHands CLI through `uvx`:
