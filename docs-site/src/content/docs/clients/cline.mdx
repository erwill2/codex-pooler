...
Cline's user-facing model metadata names are `contextWindow`, `maxInputTokens`, and `maxTokens`. If you add a manual Codex Pooler model entry in Cline settings, use `contextWindow: 372000`, `maxInputTokens: 244000`, and `maxTokens: 128000` for `gpt-5.6-terra`; Cline's compaction trigger then leaves room for a long completion inside the 372k Pooler window.
