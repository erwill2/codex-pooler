...
Pi accepts `contextWindow` and `maxTokens` for custom models; it has no `contextTokens` field. Use a 372k context window and 128k output budget for the `gpt-5.6-terra` custom entry so Pi's local context accounting matches Codex Pooler's advertised model metadata.
