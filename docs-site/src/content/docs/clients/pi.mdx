...
Pi accepts `contextWindow` and `maxTokens` for custom models; it has no `contextTokens` field. Use a 372k context window and 128k output budget for the GPT-5.6 custom entries so Pi's local context accounting matches Codex Pooler's advertised model metadata.
