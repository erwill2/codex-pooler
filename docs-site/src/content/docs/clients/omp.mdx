...
The `thinking` blocks expose all OMP effort levels for both Codex models, so role defaults can pick cheaper reasoning for background work without falling back to a non-Codex provider.
