...
OpenClaw separates the provider/native `contextWindow` from the effective runtime `contextTokens` budget. Its OpenAI `gpt-5.5` catalog publishes a 1M native window, a 272k effective cap, and a 128k output budget. The explicit compaction reserve keeps local prompt history under the remaining 144k budget before a long completion.
