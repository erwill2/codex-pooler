...
OpenClaw separates the provider/native `contextWindow` from the effective runtime `contextTokens` budget. Its OpenAI `gpt-5.6-*` catalog uses a 372k native/effective window and a 128k output budget. The explicit compaction reserve keeps local prompt history under the remaining 244k budget before a long completion. Use `gpt-5.6-luna` for background routing, keep `gpt-5.6-terra` as the primary model, and switch a session to `gpt-5.6-sol` only for heavy reasoning.
