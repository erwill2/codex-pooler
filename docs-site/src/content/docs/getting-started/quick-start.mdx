...
If you use a Pool invite instead of importing `auth.json`, the invited user must enable OpenAI's device-code approval for Codex before accepting the invite. For a personal ChatGPT account, open Settings, then Security, and turn on `Enable device code authorization for Codex`. For workspace-managed accounts, ask a workspace admin to enable device-code login for Codex in the workspace permissions. OpenAI's [Codex authentication docs](https://developers.openai.com/codex/auth) describe this prerequisite, and the hosted onboarding flow depends on it.
