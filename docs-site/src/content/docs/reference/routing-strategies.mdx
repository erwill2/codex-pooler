...
6. It orders the remaining eligible upstream accounts by assignment priority, the Pool's routing strategy, and routing preferences.
...
After eligibility, Codex Pooler orders the remaining accounts. Each Pool assignment has a routing priority from `1` to `10000`; lower numbers are preferred. Accounts at the same priority keep the order produced by the Pool's routing strategy, so the default priority of `100` preserves existing behavior until an operator configures ranks. Priority affects preference, not permission.
...
For a primary/fallback sequence, assign priorities `1`, `2`, `3`, and so on, and set the ring size high enough to include the desired fallback depth. Priority is scoped to a Pool, so one account can be primary in one Pool and a later fallback in another. Equal values form a priority tier and let the selected routing strategy decide the order within that tier.

Durable continuity remains stronger than ordinary priority: an existing Codex session, idempotent request, or affinity record can keep work on the account that owns its state. Temporary failure demotion is also applied after priority, allowing a retryably failing primary to move behind healthy fallbacks. Hard eligibility always wins; an exhausted, paused, incompatible, or otherwise ineligible primary is removed before ordering.
...
To confirm recovery without waiting for the provider's usage endpoint to catch up, the single request that triggered the redemption may act as one guarded probe against the redeemed account. A PostgreSQL-coordinated claim guarantees that at most one request across all nodes probes, and that claim is never handed to another request: if the probe fails, no second credit is spent. A successful probe makes the account temporarily routeable for a bounded confirmation window, so subsequent requests can flow while fresh quota evidence converges. Real quota evidence observed at or after the credit was consumed then confirms recovery or reblocks the account; if no such evidence arrives within the window, the account fails closed. Reconciliation applies the same evidence-only convergence, so an account that was left pending recovers automatically once the provider reports a real quota window again.
...
<table>
  ...
  <tbody>
    <tr>
      <td>Pools</td>
      <td>Pool lifecycle, Pool assignments and their routing priorities, Pool API key assignments, routing strategy, ring size, sticky websocket sessions, HTTP affinity, prompt-cache affinity, and <code>/v1</code> compatibility</td>
    </tr>
    <tr>
      <td>Upstreams</td>
...
### How do I always try one account first?

Edit the Pool, open the Upstreams step, and give the primary account priority `1`. Give its fallbacks `2`, `3`, and so on. Make sure the ring size includes the number of fallbacks you want available. Accounts with the same priority continue to use the Pool's configured routing strategy.
