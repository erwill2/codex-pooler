...
The API keys page is not an analytics surface. Use [Stats](/operators/stats/) for aggregate request, token, and cost analytics. Use [Request logs](/operators/request-logs/) for request-level investigation, including the API key label or safe prefix when available.
