...
2. upstream assignments and priorities, which decide which accounts are available and which eligible account is tried first
...
The Upstreams step selects which active upstream accounts are available to the Pool. A checked upstream becomes a Pool assignment. Each assignment also has a routing priority from `1` to `10000`; lower numbers are tried first, while equal values use the configured routing strategy as a tie-breaker. New and existing assignments default to `100`, so ranking changes nothing until you set different values. Runtime routing still applies eligibility checks before using an account.

To create a strict primary/fallback order, use priorities `1`, `2`, `3`, and so on. Keep the ring size at least as large as the fallback depth you want Codex Pooler to attempt. Priorities are per Pool, so an account's rank can differ between Pools. Session continuity can keep existing state on its owning account, and a retryably failing primary can be temporarily demoted behind a healthy fallback.
