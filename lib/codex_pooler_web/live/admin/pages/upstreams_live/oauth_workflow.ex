defmodule CodexPoolerWeb.Admin.UpstreamsLive.OAuthWorkflow do
  ...
  @spec validate_pool(Phoenix.LiveView.Socket.t(), map()) :: Phoenix.LiveView.Socket.t()
  ...
  @spec start_browser(Phoenix.LiveView.Socket.t(), map()) :: Phoenix.LiveView.Socket.t()
  def start_browser(socket, params) do
    begin_link(socket, params, &UpstreamOAuth.start_browser_oauth/3, &assign_browser_started/3)
  end

  @spec start_device(Phoenix.LiveView.Socket.t(), map()) :: Phoenix.LiveView.Socket.t()
      ...
      %OAuthFlow{id: flow_id} ->
        case UpstreamOAuth.complete_browser_oauth(
          ...
          {:ok, %{status: :completed, flow: %OAuthFlow{} = flow}} ->
            complete_link(socket, flow, reload_fun)
    ...
    case selected_oauth_pool(socket, params) do
      ...
      pool ->
        case start_oauth.(socket.assigns.current_scope, pool, start_opts(socket)) do
          {:ok, result} ->
            assign_started.(socket, pool, result)
