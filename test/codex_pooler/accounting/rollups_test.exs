defmodule CodexPooler.Accounting.RollupsTest do
  ...
  alias CodexPooler.Accounting.{HourlyModelUsageRollup, LedgerEntry, Request, Rollups}
  ...
  describe "hourly model usage rollups" do
    ...
    test "repeated recorded settlements accumulate in the same hourly model bucket" do
      ...
      assert rollup.input_tokens == 18
      assert rollup.cached_input_tokens == 5
      assert rollup.output_tokens == 14
      assert rollup.reasoning_tokens == 5
      assert rollup.total_tokens == 32
      assert decimal_string(rollup.estimated_cost_micros) == "201"
