defmodule CodexPooler.Jobs.AccountQuotaConfirmationWorkerTest do
  ...
  test "manual refresh schedules and runs its own delayed zero confirmation" do
    ...
    assert {:ok, _row} =
             Windows.record_evidence(
               ...
               weekly_evidence(
                 ...
                 DateTime.add(now, -10, :minute) |> DateTime.add(@weekly_seconds, :second)
               ),
               DateTime.add(now, -10, :minute)
             )
    ...
    |> Ecto.Changeset.change(%{
      metadata:
        row.metadata
        |> Map.put("__quota_confirmed_candidate_v1", candidate_metadata)
        |> Map.put("__quota_relative_candidate_liveness_v1", DateTime.to_iso8601(candidate_at))
    })
    |> Repo.update!()
  ...
  test "manual refresh does not schedule confirmation without a pending zero" do
    now = DateTime.utc_now() |> DateTime.truncate(:microsecond)
    upstream = start_usage_upstream(43, DateTime.add(now, @weekly_seconds, :second))
    %{identity: identity, assignment: assignment} = assignment_fixture(upstream)
    Repo.delete_all(Oban.Job)
  ...
  defp weekly_evidence(used_percent, observed_at, reset_at) do
    %{
      ...
      metadata: %{"reset_after_seconds" => @weekly_seconds}
    }
  end
              ...
              %{
                "rate_limit" => %{
                  "primary_window" => %{
                    ...
                    "reset_after_seconds" => @weekly_seconds
                  }
                }
              }}
         }}
